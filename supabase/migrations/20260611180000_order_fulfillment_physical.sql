-- Fulfillment + delivery-method must follow the order's PHYSICAL CONTENT,
-- not whether a shipping NAME string happened to be filled in.
--
-- Before: create_pending_order set delivery_method='shipping' only when a
-- shipping_name was passed, so a physical order with a blank name fell
-- through to 'download'; mark_order_paid then keyed fulfillment off
-- delivery_method='shipping' and marked such orders 'completed' ("Выполнен")
-- the instant they were paid — even though they still need to be shipped.
--
-- After: create_pending_order detects physical items in the cart
-- (PrintBook / Book2.0 / a physical BoxSet) and sets delivery_method='shipping'
-- accordingly; mark_order_paid sets fulfillment_status='processing' ("В обработке")
-- whenever the order contains a physical line (box sets are already expanded
-- into PrintBook/Book2.0 OrderItems at order time), else 'completed'.
...
BEGIN
  ...
  FOR cart_row IN SELECT * FROM "Cart" WHERE user_id = v_user_id LOOP
    ...
    -- Does this line require physical shipping? (Box sets are physical when any
    -- contained edition is — box_set_is_physical resolves that from BoxSetBooks.)
...
CREATE OR REPLACE FUNCTION public.mark_order_paid(p_inv_id integer, p_out_sum text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
