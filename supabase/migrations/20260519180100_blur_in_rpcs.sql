-- Surface image blur placeholders through the catalog RPCs.
--
-- - get_catalog_books        + title_cover_blur
-- - get_similar_books        + title_cover_blur
-- - search_books             + title_cover_blur
-- - get_catalog_book_by_slug + title_cover_blur, photo_blur inside title_authors
--
-- `book_photos_blurs` is read separately by getBookPhotos (which already
-- needs a Storage listing) and is not surfaced here.

-- ─── get_catalog_books ──────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS get_catalog_books(int, int, text, text, text, numeric, numeric, text, int[], text[], text[], text[]);
...
DROP FUNCTION IF EXISTS get_similar_books(integer);
...
DROP FUNCTION IF EXISTS search_books(text, int, int);
