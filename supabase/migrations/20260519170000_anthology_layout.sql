-- Anthology layout support.
--
-- Marks the six compilation titles (Худшее, Худшее-2, Адвент-календарь, Я есть
-- Россия, Абзац, Локал) so the book detail page can switch to the dedicated
-- "Авторы" list layout instead of rendering one full "Об авторе" section per
-- contributor (impractical for 50+ contributors).
--
-- Also extends get_catalog_book_by_slug to return title_is_compilation so the
-- client doesn't need a second query.
...
-- ─── RPC: get_catalog_book_by_slug (add title_is_compilation) ───────────────
