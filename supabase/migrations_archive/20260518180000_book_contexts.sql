...
--   * BookContexts table — per-title list of supplementary materials
--     (interviews, articles, events, trivia). Each item has a heading,
--     body text, and an optional external URL.
--   * get_catalog_book_by_slug RPC extended to return title_contexts jsonb.

-- ─── BookContexts table ──────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS "BookContexts" (
...
CREATE INDEX IF NOT EXISTS bookcontexts_title_id_idx ON "BookContexts" (title_id);

ALTER TABLE "BookContexts" ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public read book contexts" ON "BookContexts";
CREATE POLICY "Public read book contexts"
  ON "BookContexts" FOR SELECT USING (true);

-- ─── RPC: get_catalog_book_by_slug (add title_contexts) ─────────────────────
