...
- [x] `SubscriptionCard` (B1+B2) — moved out of the `'use client'` carousel module into its own directive-less file + a client `SubscriptionConnectButton` leaf. Net effect: the **desktop grid** (`SubscriptionsSection`, server) now renders server cards (before, importing the card from the client carousel module made even the grid cards client); the **mobile Swiper carousel** cards stay client (unavoidable without the children-prop pattern, which Swiper's `SwiperSlide` model makes impractical — and that's the documented B2 trade-off).
...
- [~] `Header` (B5) — **intentionally left client**: it's legitimately interactive throughout — reactive active-link highlighting (`usePathname`, must update on SPA nav), Radix dropdowns, the mobile dialog, and the cart badge. The only server-extractable piece is the logo `<Link>` (negligible), so splitting adds indirection for ~nothing.
...
- [x] **Cloudflare `email-decode.min.js`** — user disabled Email-Obfuscation in the dashboard. **Correction:** I had overstated this — Cloudflare injects the script with `defer`, so it does **not** block FCP/LCP (Cloudflare's support was right). The Lighthouse "2.2 s longest chain" was its *position* in the dependency graph (loads late *because* deferred), not 2.2 s of blocking. Disabling it is a minor cleanup (one fewer request + bit of main-thread JS), not the headline win. Render-blocking CSS is the real #1 lever.
- [x] **Forced reflow 90 ms** = Swiper `updateSize`/`updateAutoHeight` → deferred the below-fold mobile `SubscriptionsCarousel` via a client `next/dynamic({ssr:false})` wrapper (`SubscriptionsCarouselLazy`), moving its Swiper init off the LCP window. Hero Slider kept eager (it's the LCP; its `autoHeight` is a deliberate Firefox height-collapse fix — left as-is).
...
### Phase 7 — additional PSI insights (round 2, 2026-06-17)
After the box-set WebP fix dropped the home doc 2.7 MB → 0.21 MB, further real-PSI rounds surfaced:
- [x] **LCP request discovery** (fetchpriority FAILED) → `priority` doesn't set fetchpriority in this Next; added explicit `fetchPriority="high"` on the first hero slide → preload now ships `fetchpriority=high` (verified live).
- [x] **Improve image delivery (546 KiB)** → hero Slider had **no `sizes`** → served w=750 for a ~174px display (4.3×, incl. the LCP). Added `sizes` (→ w=384, ~66% smaller LCP image) + enabled AVIF (`formats:['image/avif','image/webp']`, verified sharp encodes AVIF on the alpine prod container). BookCard/BoxSet already had `sizes` (retina-correct); AVIF trims their bytes.
- [x] **Render-blocking CSS** → `inlineCss` (re-enabled now the doc is lean): 0 blocking stylesheet requests (the only App-Router mechanism; per-component split/stream does NOT make `<link>`s non-blocking; `optimizeCss`/Beasties is Pages-Router-only — confirmed `postProcessHTML` is wired into `render.js` only).
- [x] **Network dependency tree / fonts** → confirmed next/font is correct (swap + subsets + variable, so fonts don't block FCP). Chequeblack `.ttf`(44 KB)→`.woff2`(12 KB); Montserrat weights `400/500/600/700`→`400/600/700/900` (500 was unused, 900 was faux-bolded).
- [x] **Unused preconnect** → removed the `api.mildfire.dev` preconnect: covers load via same-origin `/_next/image` (optimizer fetches the upstream server-side), so the browser never connects to it on the critical path; only post-hydration cart/likes do.
- [x] **Cloudflare Early Hints (103)** → enabled (dashboard). Sends the font-preload `Link:` headers during the ~1 s TTFB so fonts fetch before the HTML — verified `HTTP/2 103` live. (Rocket Loader confirmed OFF.)
