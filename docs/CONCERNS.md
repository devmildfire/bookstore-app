...
| CA5 | ✅ Fixed | All 69 hardcoded hex replaced with tokens across 59 SCSS files. 7 new tokens added to `params.scss`: `$color-text-on-accent` (white on accent buttons), `$color-form-success`/`$color-form-error` (admin form feedback), `$color-control-muted`/`$color-label-muted` (muted UI), `$color-sidebar-gradient-start`/`$color-sidebar-gradient-end` (sidebar gradient), `$color-toast-start` (toast gradient). `tsc` + lint verified. |
